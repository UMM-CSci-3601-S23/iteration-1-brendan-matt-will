...
import { DonorListComponent } from './food-shelf-donor-list.component' ;
...
describe('Message list', () => {
  ...
  let donorList: DonorListComponent;
  ...
  it('contains all the messages', () => {
    expect(donorList.serverFilteredMessages.length).toBe(3);
  });

  it('contains a message with name \'Chris\'', () => {
    expect(donorList.serverFilteredMessages.some((message: Message) => message.name === 'Chris')).toBe(true);
  });

  it('contain a message with name \'John\'', () => {
    expect(donorList.serverFilteredMessages.some((message: Message) => message.name === 'John')).toBe(true);
  });

  it('doesn\'t contain a message with name \'Santa\'', () => {
    expect(donorList.serverFilteredMessages.some((message: Message) => message.name === 'Santa')).toBe(false);
  });

  it('has one message with body "I dont want anything"', () => {
    expect(donorList.serverFilteredMessages.filter((message: Message) => message.body === 'I want beans').length).toBe(1);
  });
});
